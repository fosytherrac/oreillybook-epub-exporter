...
- **Full-page manager** (`manager.html/js/css`) — a roomy catalog browser opened in its own tab (also registered as the extension's `options_ui`). Shares the catalog-browsing logic with the popup via `lib/catalog-ui.js`; only the layout (a responsive grid of cards) differs, in CSS. Also offers **Export ALL (JSON)**: page 0 reports the `total`, then every remaining page is fetched **concurrently** through `Downloader._adaptivePool` (metadata only — no cover images), deduped by ISBN, and saved as one `.json` file. This is the fast path for enumerating a whole category/search (or the entire ~49k-title catalog with `query=*`) without rendering thousands of cards. It probes a large page size (`limit=500`) and only adopts it if O'Reilly actually returns that many, otherwise falling back to `limit=100` — because a silently-capped limit paginated by the requested offset would skip records. The same fetched (or an **imported**) list can be stored as a **local index** (via `lib/catalog-store.js`) that the manager searches instantly client-side (multi-token AND over title/authors/publisher/ISBN, paginated), persisted in IndexedDB across sessions. From the local index, books can be added to a **bulk download queue** (per-row ＋ or "Queue all matches") that downloads them **sequentially** (EPUB only; a 1.2s gap between books; the AIMD handles per-file rate limits), continuing past failures (a no-files title is marked failed and skipped), with per-book + overall progress, Stop, and Retry-failed. While the queue runs it holds the shared `busy` flag so single downloads / index builds can't overlap it.
...
- `lib/catalog-ui.js` — Shared catalog-browser UI factory (`CatalogUI.init(els, opts)`): wires search, result rendering, per-book download, and CSV export onto a given set of DOM elements. Used by both `popup.js` and `manager.js` so the browsing logic lives in one place; the two pages differ only in CSS layout. Emits identical `.book-row` markup (via the reusable `CatalogUI.renderBooks(container, books, onDownload)` static, also used by the manager's local index). Not a content script — loaded via `<script>` in the popup and manager pages.
- `lib/catalog-store.js` — `CatalogStore` (IndexedDB wrapper: `save`/`load`/`clear` one record) persisting the manager's **local catalog index** — the full metadata list built from Export-ALL or an imported JSON — so the whole catalog (~49k books) can be searched instantly and offline without per-keystroke API calls. Manager-only.
...
- **Adaptive fetch pacing** (`Downloader._adaptivePool`): images and chapters are fetched through an AIMD (TCP-style) concurrency pool — it keeps the pipe full, additively increases concurrency while requests succeed, and **halves** the in-flight count the instant a worker reports a 403/429 (`onRateLimit`, fired by `Fetcher._fetchWithRetry`), converging just under O'Reilly's real limit rather than bursting into it. It returns results index-aligned (so chapter order is preserved even though completion order isn't), turns ordinary fetch errors into `null` (→ placeholder / omitted image), and rejects the whole run on `AbortError`/`SESSION_EXPIRED`. O'Reilly rate-limits *concurrent bursts* (confirmed from a real HAR: 5 simultaneous chapter requests → all 403, recovered on retry). Images are fetched in the pool then named/written sequentially (deterministic filenames); chapters are pool-fetched then processed in order. `fetcher.js`'s per-request retry/backoff recovers the individual rejected requests. (Earlier versions used fixed batches-of-2 + 500ms/1000ms delays that assumed an unmeasured rate limit.)
