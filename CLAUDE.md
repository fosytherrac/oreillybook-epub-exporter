...
- **Download engine** (`lib/downloader.js`) — `Downloader.download({isbn, apiBase, signal, onProgress, fallbackTitle})` does the manifest fetch (with empty-manifest retry), image/CSS collection, chapter processing, and ZIP assembly, returning `{blob, filename, title}`. It's context-agnostic: the content script calls it with `apiBase=''`; the full-page manager calls it with `apiBase='https://learning.oreilly.com'` and builds the EPUB **in-page** (extension pages inherit host_permissions, so cross-origin fetches carry O'Reilly cookies — no book tab is opened). The popup can't build in-page (its window closes), so its catalog downloads still go through the background's `downloadBook` (open tab + auto-export).
